...
**当前版本：v0.2.21** — 回复逻辑改为纯文本驱动：`@personaName` 触发 AI 回复，`Reply To #N` 引用特定评论，不再依赖外部嵌套状态。
...
| **多代理更新系统** | 内置镜像代理池（ghproxy.net / ghproxy.com / gh.llkk.cc / gh.ddlc.top + 直连 GitHub 兜底），启动时**并行测速所有代理**，自动选最快代理检查更新与下载 APK；测速日志可见各代理响应耗时（ms） |
